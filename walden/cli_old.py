...
def delete_journal(journal_name):
    ...
    if journal_name in meta_data:
        ...
        while not (confirmation.lower() == "y" or confirmation.lower() == "n"):
            confirmation = input(
                f'Are you sure you want to delete "{journal_name}"? (Y/N): '
            )

        if confirmation.casefold() == "y".casefold():
            path = pathlib.Path(meta_data.pop(journal_name))
            pickle.dump(
                meta_data, open(f"{DEFAULT_JOURNALS_PATH}/.journals_info", "wb")
            )
            shutil.rmtree(path)
...
def process_today(journal_name):
    ...
    if journal_name in meta_data:
        ...
        print(
            f'No journal named "{journal_name}" exists. See walden -list for list of journals.'
        )
