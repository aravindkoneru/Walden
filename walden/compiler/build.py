...
def parse_entries(path):
    ...
    years = [
        str(f)[str(f).rindex("/") + 1 :] for f in years_folder.iterdir() if f.is_dir()
    ]
    years.sort()
    ...
    for year in years:
        ...
        entries[year] = [
            str(f)[str(f).rindex("/") + 1 :]
            for f in months_folder.iterdir()
            if f.is_dir()
        ]
        entries[year].sort()
    ...
    for year in entries:
        for month in entries[year]:
            with open(f"{entries_path}/{year}/{month}.tex", "w") as monthly_entry:
                monthly_entry.write(
                    dedent(format_month(f"{entries_path}/{year}/{month}", year, month))
                )
...
def format_month(month_path, year, month):
    ...
    daily_entries = [
        str(f)[str(f).rindex("/") + 1 :] for f in month_folder.iterdir() if f.is_file()
    ]
    daily_entries.sort()
